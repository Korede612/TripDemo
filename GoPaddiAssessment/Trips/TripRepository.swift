...
struct CreateTripRequest: APIRequest {
    ...
    init(destination: String, startDate: Date, endDate: Date) {
        body = Body(destination: destination, startDate: startDate, endDate: endDate)
    }
}
...
protocol TripRepositoryProtocol {
    ...
    func createTrip(destination: String, startDate: Date, endDate: Date) async throws -> Trip
}
...
final class TripRepository: TripRepositoryProtocol {
    ...
    func createTrip(destination: String, startDate: Date, endDate: Date) async throws -> Trip {
        try await client.send(CreateTripRequest(destination: destination, startDate: startDate, endDate: endDate))
    }
}
